...
class InputFileParser(object):
	...
	def parse(self, inputfile):
		...
		csv_values = self._parse_csvfile(inputfile, self.separator, self.line_separator, self.FILE_COLUMNS, self.target_row)
		if csv_values == None:
			return []
	...
	def _parse_csvfile(self, csv_file, separator, line_separator, columns, target_row):
		...
		try:
			...
			with csv_file.open('r', encoding=self.charset) as infile:
				csvFile = csv.reader(infile, delimiter=separator, lineterminator=line_separator, dialect='excel')
				...
				for values in csvFile:
					...
					if len(values) != len(columns):
						print('invalid format{}: {}'.format(str(csv_file), line))
						continue
